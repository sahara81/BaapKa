...
def detect_hinglish(text: str) -> bool:
    """Basic input language check — Hinglish/Hindi detect."""
    if not text:
        return False

    # Devanagari script detect
...
def generate_premium_line(text: str) -> str:
    """Language-aware premium detailed reply with emojis."""
...
def get_not_found_response(context: ContextTypes.DEFAULT_TYPE, source_text: str) -> str:
    """
    FINAL AUTO MODE:
    - 1st request: premium detailed
    - 2nd request: premium detailed (different line)
    - 3rd+ requests: short direct response with emoji
    ALWAYS includes spelling/database possibility.
    """

    count = context.chat_data.get("nf_counter", 0)
    context.chat_data["nf_counter"] = count + 1

    # Premium tone for first two times
    if count < 2:
        return generate_premium_line(source_text)

    # From 3rd time onward → short tone
    if detect_hinglish(source_text):
...
async def handle_searching_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Dusre request-bot ka 'Searching...' message aaya.
    Agar auto-timeout ke andar answer nahi aata,
    to hum 'not found, spelling/database reason' reply dete hain.
    """
    ...
    async def timeout_check(search_msg_id: int, request_text: str):
        # Auto adjust based on request complexity
        ...
        # still unanswered → send not found reply (auto tone + auto lang)
...
async def mark_search_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Agar dusra bot actual result reply kare, to pending search ko answered mark karo.
    """
    msg = update.effective_message
    if not msg or not msg.reply_to_message:
        return
    ...
    # direct reply to searching message
    ...
    # reply to original request message (map by orig_id)
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle other bots (including request bot)
    if user.is_bot:
        username = (user.username or "").lower()

        lower_list = [u.lower() for u in OTHER_BOT_USERNAMES]
        if username in lower_list:
            if SEARCH_TRIGGER_TEXT in text:
    ...
    if context.chat_data.get("nsfw_enabled", True):
        if any(nw in clean for nw in NSFW_PATTERNS) or any(e in text for e in NSFW_EMOJI):
            ...
            warn = await msg.reply_text("🚫 NSFW content removed.")
    ...
    if is_link or is_tag_spam:
        add_log(context, f"PROMO_BLOCK user={user.id} name={user.full_name!r} text={raw_text!r}")
        await force_delete(msg, context)
        warn = await msg.reply_text("🚫 Promotion / tag spam removed.")
        asyncio.create_task(delete_after(context.bot, warn.chat.id, warn.message_id, 5))
        return
