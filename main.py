...
OWNER_ID = int(os.getenv("OWNER_ID", "0"))  # apna Telegram user id (env se aayega)

DELETE_DELAY = int(os.getenv("DELETE_DELAY", "300"))
PORT = int(os.getenv("PORT", "10000"))  # Render / Koyeb dega
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # group admin: only when in group/supergroup
    is_group_admin = False
    if not in_private:
        is_group_admin = await is_admin(update, context)

    show_admin_view = is_owner or (not in_private and is_group_admin)
